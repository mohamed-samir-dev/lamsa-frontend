    ...
    <section className="mb-14">
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-2xl flex items-center justify-center shadow-lg border border-[#BC9255]/20" style={{ background: "linear-gradient(135deg, #0A1825, #1a2d42)" }}>
            <span className="text-[#BC9255] font-bold text-[13px]">S</span>
          </div>
          <div>
            <h2 className="text-base sm:text-lg font-black" style={{ color: "#0A1825" }}>تسوّق حسب الطراز</h2>
            <p className="text-[10px] sm:text-[11px] text-[#0A1825]/40">اضغط على الطراز للذهاب لصفحته</p>
          </div>
        </div>
          ...
          return (
            ...
            >
              <Link
                ...
                className="relative flex-shrink-0 w-[130px] sm:w-[175px] rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-400 group block border border-transparent hover:border-[#BC9255]/40"
                style={{ background: "linear-gradient(180deg, #FFFFFF 0%, #F8F5F1 100%)", boxShadow: "0 4px 20px rgba(10,24,37,0.06)" }}
              >
                <div className="relative w-full h-[95px] sm:h-[140px] overflow-hidden">
                  {img ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="text-[#0A1825]/15 font-bold text-2xl">S</span>
                    </div>
                  )}
                  <div className="absolute top-2 left-2 sm:top-2.5 sm:left-2.5 w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[8px] sm:text-[9px] font-bold" style={{ backgroundColor: "#0A1825", color: "#BC9255" }}>
                    {categoryCounts[cat.slug]}
                  </div>
                </div>
                <div className="px-2 py-2 sm:px-3 sm:py-3 text-center border-t border-[#EBE6E2]/60">
                  <p className="text-[11px] sm:text-[13px] font-bold truncate" style={{ color: "#0A1825" }}>{cat.label}</p>
                  <p className="text-[9px] sm:text-[11px] mt-0.5 sm:mt-1 text-[#0A1825]/50 truncate">{cat.desc}</p>
                </div>
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl" style={{ background: "radial-gradient(circle at 50% 30%, rgba(188,146,85,0.08), transparent 70%)" }} />
              </Link>
            </motion.div>
          );
        })}
